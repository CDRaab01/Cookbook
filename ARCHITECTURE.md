...
- **`lists/merge.py`** — shopping-list merge math. Merge identity is the **normalized name only**
  ...
  are filtered at add-recipe, and **cooking-only measures** (`shopping_measure` drops cup/tbsp/tsp/
  pinch/dash/clove/ml/l) are stripped when a recipe goes onto a list — you buy a bottle, not
  "0.25 cup"; weights, counts, and package units (lb/oz/can/bag…) pass through. Exhaustively
  table-driven-tested; **clients never merge independently** — every path into a list (recipe add,
  plan-to-list, manual add, undo rebuild) goes through this module.
- **`lists/categorize.py`** — store-category guesser (fallback behind `item_history` recall).
  ...
  *egg*, "chipotle" isn't *chip*. Keywords are matched against both the raw and merge-normalized
  name with a tolerant trailing plural. **Product-form head words win** over a longer word in
  front of them (`_PRIORITY`: juice/lemonade/smoothie/kombucha → beverages), so "pineapple juice"
  is a beverage, not produce. Migration `0019` did a **one-time re-sort** of existing
  `shopping_list_items` + `item_history` into the wider set, but only where the stored category
  equaled the *old* guesser's output (auto-assigned) — manual picks were left untouched. Migration
  `0020` re-ran that same safe re-sort (picking up the beverage head-word fix) and stripped
  cooking-only measures from existing list items.
