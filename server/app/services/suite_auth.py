"""Suite SSO — trade a Dragonfly-issued suite token for a Cookbook session (BROKER.md Phase 2b).
...
async def verify_cross_app_token(token: str) -> str | None:
    ...
    Ported from Spotter's provider-side helper — but Cookbook's provider surfaces are
    post-HS256-retirement (CROSS-APP.md: new surfaces are RS256-only), so the caller has NO
    legacy fallback: None means 401. Validates against the same JWKS Cookbook already trusts
    for SSO; requires ``aud="cross-app"``."""
...
async def suite_login(db: AsyncSession, suite_token: str) -> TokenResponse:
    ...
    if user is None:
        # First time in this app → link a fresh account by email. The suite is this user's login,
        # so there's no password yet; a random hash keeps the column valid (and unusable) until
        # they set one via the normal reset flow.
        user = User(
