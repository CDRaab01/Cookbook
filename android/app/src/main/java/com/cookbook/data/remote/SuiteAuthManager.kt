 ...
 * identity server (BROKER.md Phase 2c). The flow: launch a Custom Tab to the identity server, get a
 * suite token, then trade it at Cookbook's own `/auth/suite` for a Cookbook session. Because the
 * Custom Tab shares the system browser's session, once you've signed in there for one suite app the
 * others skip the login — that's the actual single-sign-on.
 */
@Singleton
class SuiteAuthManager @Inject constructor(
     ...
     * Cookbook's `/auth/suite` for a Cookbook session, and persist it. Throws on cancel/failure.
     */
    ...
    private companion object {
        ...
        const val CLIENT_ID = "cookbook"
        const val REDIRECT_URI = "com.cookbook:/oauth2redirect"
    }
}
